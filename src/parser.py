...
class Parser:
    ...
    def __init__(self):
        ...
        # block is a stmt_list (statement list) enclosed by 2 curly brackets
        @pg.production("block : L_BRACE stmt_list R_BRACE")
        def stmt_list_block(s):
            """parse block of statement list"""
            return AST.Block_Stmt(s[1])

        # add the newline
        @pg.production("block : L_BRACE NEWLINE stmt_list R_BRACE")
